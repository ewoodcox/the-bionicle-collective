/**
 * Krana collection store (file-based for local dev, in-memory fallback in Workers).
 * Uses src/data/krana-store.json when Node fs is available.
...
async function readStore(): Promise<string[]> {
  try {
    ...
    const STORE_PATH = join(__dirname, '..', 'data', 'krana-store.json');
...
export async function getKranaOwnedIds(): Promise<string[]> {
  return readStore();
}

export async function setKranaOwnedIds(ids: string[]): Promise<string[]> {
