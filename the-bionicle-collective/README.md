...
Output is in `dist/` (ignored in git; produced by `npm run build`). This project targets **Cloudflare Pages** (SSR + R2 for collection data, KV for admin session). See **`docs/DEPLOY-CLOUDFLARE-GIT.md`** for deployment, custom domains, and bindings.
